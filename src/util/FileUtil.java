...
public class FileUtil {
	...
	public static MultipartRequest upload(
		...
		/*
		 매개변수1 : 사용자가 요청 정보가 저장된 request 내장객체
		 매개변수2 : 파일이 업로드 될 서버의 물리적경로
		 그 외 업로드 할 파일의 최대 용량, 인코딩방식, 파일명 중복을 처리 할 클래스를
	...
	public static void deleteFile(HttpServletRequest req, String directory, String filename) {
		...
		//만약 해당경로에 파일이 존재한다면 삭제처리(있겠지 바부야)
		if(f.exists()) {
			f.delete();
		}
	}
	...
	//파일 다운로드 로직
	/*
	 String ofileName : 첨부파일의 원본파일명 
	 String sfileName : 서버에 저장된 변경된 파일명
	 */
